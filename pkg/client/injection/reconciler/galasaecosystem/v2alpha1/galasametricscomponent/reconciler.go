...
// controller reconciling v2alpha1.GalasaMetricsComponent.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v2alpha1.GalasaMetricsComponent. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v2alpha1.GalasaMetricsComponent) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v2alpha1.GalasaMetricsComponent.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v2alpha1.GalasaMetricsComponent. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v2alpha1.GalasaMetricsComponent) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v2alpha1.GalasaMetricsComponent if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v2alpha1.GalasaMetricsComponent.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v2alpha1.GalasaMetricsComponent) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v2alpha1.GalasaMetricsComponent) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v2alpha1.GalasaMetricsComponent resources.
type reconcilerImpl struct {
	...
	Lister galasaecosystemv2alpha1.GalasaMetricsComponentLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister galasaecosystemv2alpha1.GalasaMetricsComponentLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.GalasaMetricsComponents(s.namespace)
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, existing *v2alpha1.GalasaMetricsComponent, desired *v2alpha1.GalasaMetricsComponent) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.GalasaV2alpha1().GalasaMetricsComponents(desired.Namespace)
		...
		updater := r.Client.GalasaV2alpha1().GalasaMetricsComponents(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v2alpha1.GalasaMetricsComponent) (*v2alpha1.GalasaMetricsComponent, error) {

	getter := r.Lister.GalasaMetricsComponents(resource.Namespace)
	...
	patcher := r.Client.GalasaV2alpha1().GalasaMetricsComponents(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v2alpha1.GalasaMetricsComponent) (*v2alpha1.GalasaMetricsComponent, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v2alpha1.GalasaMetricsComponent, reconcileEvent reconciler.Event) (*v2alpha1.GalasaMetricsComponent, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
