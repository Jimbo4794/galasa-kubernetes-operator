...
func (c *Reconciler) ReconcileKind(ctx context.Context, p *v2alpha1.GalasaMetricsComponent) pkgreconciler.Event {
	logger := logging.FromContext(ctx)
	metrics := ecosystem.Metrics(p, c.GalasaEcosystemClientSet)
	objects := metrics.GetObjects()
	logger.Infof("Reconciling Metrics")
	...
	if dep.Status.ReadyReplicas == 1 {
		...
		c.GalasaEcosystemClientSet.GalasaV2alpha1().GalasaMetricsComponents(p.Namespace).UpdateStatus(ctx, p, v1.UpdateOptions{})
		return nil
	} else {
		p.Status = v2alpha1.ComponentStatus{
			Ready: false,
